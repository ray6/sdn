...
class SimpleSwitch13(app_manager.RyuApp):
	...
	def __init__(self, *args, **kwargs):
		...
		self.ip_addrs = ['10.0.0.1', '10.0.0.2', '10.0.0.3', '10.0.0.4'
             			 ,'10.0.0.5', '10.0.0.6']
        	self.hw_addrs = ['00:00:00:00:00:01', '00:00:00:00:00:02','00:00:00:00:00:03','00:00:00:00:00:04', 
	...
	def path_install(self,ev):
		for i, src in enumerate(self.vtable):
			for j, dst in enumerate(self.vtable):
				if src != dst:
					...
					for k, sw in enumerate(self.switches):
						if sw in path:
								...
								match = self.switches_dp[k].ofproto_parser.OFPMatch(
	...
	#	self.logger.info("packet in dpid=%s src=%s dst=%s in_port=%s",dpid,src,dst,in_port)
		...
		if src not in self.directed_Topo:	#learn it
			self.directed_Topo.add_node(src)	#add a node to the graph
			...
			self.directed_Topo.add_edge(src,dpid)	#Add a link from the node to it's edge switch
			
			if src in self.vtable:
				self.host_cnt+=1
				if self.host_cnt == 6: 		
					
					self.path_install(ev)
					
		if dst in self.directed_Topo:
			path = nx.shortest_path(self.directed_Topo, src, dst)	#get shortest path
			
			if dpid not in path:
				return
			next = path[path.index(dpid)+1]	#get next hop
			print 'next: ',next
			out_port = self.directed_Topo[dpid][next]['port']	#get output port
